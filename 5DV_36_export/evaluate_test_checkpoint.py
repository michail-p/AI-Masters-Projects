...
BATCH_SIZE = 16
MODEL_PATH = "affectnet_emotion_cnn.keras"
...
def evaluate_split(split_name):
    dataset = DataLoader(os.path.join(DATA_DIR, split_name), CLASS_MAP, batch_size=BATCH_SIZE)
    model = tf.keras.models.load_model(MODEL_PATH)

    loss, accuracy = model.evaluate(dataset, verbose=0)
    print({f"{split_name}_loss": float(loss), f"{split_name}_accuracy": float(accuracy)})
